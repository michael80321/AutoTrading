...
export default function CryptoPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/crypto/snapshot`);
      if (!res.ok) throw new Error("API error");
      const d = await res.json();
      setData(d);
      setLastUpdate(new Date().toLocaleTimeString("zh-TW", { timeZone: "Asia/Taipei" }));
    } catch {
      // silent fail on polling, only show error on first load
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const portfolio = data?.portfolio?.crypto_pool;
        ...
        {!loading && data && (
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="xl:col-span-1 space-y-4">
              ...
              {portfolio && <PortfolioCard info={portfolio} pool="crypto" />}
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-300 mb-3">
                  開放倉位
                  {positions.length > 0 && (
                    <span className="ml-2 text-sm text-yellow-400 font-normal">{positions.length} 筆</span>
                  )}
                </h2>
                <PositionTable positions={positions} pool="crypto" />
              </div>
