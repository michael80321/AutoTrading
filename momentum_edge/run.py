...
def _run_one(df: pd.DataFrame, N: int, ratio: float) -> dict:
    ...
    # 5+6. 回測 + 基準（IS / OOS / 全期 各跑一次，分開）
    strat_full = backtest(df, signal)
    bh_full = buy_and_hold(df)
    strat_is = backtest(is_df, signal.loc[is_df.index])
    bh_is = buy_and_hold(is_df)
    strat_oos = backtest(oos_df, signal.loc[oos_df.index])
    bh_oos = buy_and_hold(oos_df)
